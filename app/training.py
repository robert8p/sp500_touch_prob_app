...
def build_training_dataset(
    ...
    for di,d in enumerate(days):
        ...
        if err5: return np.empty((0,len(FEATURE_NAMES))), np.array([]), np.array([]), np.array([]), [], f"bars5 fetch failed {d}: {err5}"
        bars1, err1, _ = client.get_bars(symbols_all, "1Min", start_utc=open_utc, end_utc=close_utc)
        if err1: return np.empty((0,len(FEATURE_NAMES))), np.array([]), np.array([]), np.array([]), [], f"bars1 fetch failed {d}: {err1}"
        ...
        for sym in symbols:
            ...
            ema_diff_pct = np.where(price>0, (ema_fast-ema_slow)/price, 0.0)
            ...
            obv_slope_norm = np.where(medv>0, obv_slope/np.maximum(1.0, medv), 0.0)
            ...
            for i in range(0, c5.size):
                ...
                tod=float(np.clip((390.0-mins_to_close)/390.0,0.0,1.0))
    ...
    for cw in [None, "balanced"]:
        pipe=Pipeline([
            ...
            ("clf", LogisticRegression(
                solver="lbfgs", max_iter=800, n_jobs=1,
                class_weight=cw
            )),
        ])
        pipe.fit(X_tr, y_tr)
...
def run_training(settings: Settings, symbols: List[str]) -> Dict[str, object]:
    ...
    X,y1,y2,day_idx,days,err = build_training_dataset(
        client, symbols, settings.train_lookback_days, settings.timezone, settings.scan_interval_minutes
    )
    if err:
