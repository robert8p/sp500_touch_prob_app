...
def _atr(h: np.ndarray, l: np.ndarray, c: np.ndarray, period: int=14) -> np.ndarray:
    if c.size<2: return np.zeros_like(c, dtype=float)
...
def _adx(h: np.ndarray, l: np.ndarray, c: np.ndarray, period: int=14) -> np.ndarray:
    n=c.size
    if n<period+2: return np.zeros_like(c, dtype=float)
...
class AppendSplineFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        X = np.asarray(X, dtype=float)
        S = self._spline.transform(X[:, self.idxs])
...
def _fit_best_calibrator(raw: np.ndarray, y: np.ndarray, min_samples: int) -> Calibrator:
    # stable selection: if small sample, use Platt
    raw = np.asarray(raw, dtype=float).reshape(-1)
    y = np.asarray(y, dtype=int).reshape(-1)
    if raw.size < max(min_samples, 200):
        return _fit_platt(raw, y)
    # deterministic split for selection (avoid overfitting isotonic)
    idx = np.arange(raw.size)
    split = int(0.8*raw.size)
    fit_idx = idx[:split]
    eval_idx = idx[split:] if split < raw.size else idx[:0]

    pl = _fit_platt(raw[fit_idx], y[fit_idx])
    pl_p = pl.predict(raw[eval_idx]) if eval_idx.size else pl.predict(raw)
    pl_b = _brier(y[eval_idx].astype(float) if eval_idx.size else y.astype(float), pl_p.astype(float))

    iso = _fit_isotonic(raw[fit_idx], y[fit_idx])
    if iso is None:
        return _fit_platt(raw, y)
    iso_p = iso.predict(raw[eval_idx]) if eval_idx.size else iso.predict(raw)
    iso_b = _brier(y[eval_idx].astype(float) if eval_idx.size else y.astype(float), iso_p.astype(float))
...
def _fit_bucket_calibrators(raw: np.ndarray, y: np.ndarray, ttc_frac: np.ndarray, min_bucket_samples: int) -> Tuple[Dict[str, Calibrator], Dict[str, str]]:
    raw = np.asarray(raw, dtype=float).reshape(-1)
    y = np.asarray(y, dtype=int).reshape(-1)
    ttc_frac = np.asarray(ttc_frac, dtype=float).reshape(-1)
    calibs: Dict[str, Calibrator] = {}
    methods: Dict[str, str] = {}

    # global fallback
    cal_global = _fit_best_calibrator(raw, y, min_bucket_samples)
    calibs["global"] = cal_global
    methods["global"] = cal_global.method
    ...
    for name, _, _ in BUCKETS:
        ...
        calibs[name] = c
        methods[name] = c.method
    return calibs, methods

def _bucket_priors(y: np.ndarray, ttc_frac: np.ndarray) -> Dict[str, float]:
    ...
    for di, d in enumerate(days):
        ...
        for sym in symbols:
            ...
            for i in range(0, c5.size):
                ...
                # microstructure rolling medians (exclude current bar)
                ...
                dvol_log=float(np.log1p(max(0.0, dvol_med)))
                range_pct_med=float(max(0.0, range_med))
                wick_atr_med=float(max(0.0, wick_med))

                # interactions
                int_mom_rvol=float(ret30*(rvol_tod-1.0))
                int_mom_vwap=float(ret30*float(vwap_loc[i]))
                int_trend_adx=float(float(ema_diff_pct[i])*float(adx_arr[i]))
                int_mom_ttc=float(ret30*ttc_frac)

                feats=np.array([
                    float(ret5), float(ret30), float(rel_strength), float(ema_diff_pct[i]), float(adx_arr[i]), float(atr_pct[i]),
                    float(rv[i]), float(rvol_tod), float(obv_slope_norm), float(vwap_loc[i]), float(donch[i]),
                    float(ttc_frac), float(log_m), float(tod_frac),
                    float(dvol_log), float(range_pct_med), float(wick_atr_med),
                    float(int_mom_rvol), float(int_mom_vwap), float(int_trend_adx), float(int_mom_ttc)
                ], dtype=float)
...
def _make_base_pipeline(C: float, l1_ratio: float, class_weight: Optional[str]) -> Pipeline:
    # Append spline basis for key nonlinear drivers (keeps original features too)
...
def _select_and_train_bundle(
    ...
    # priors estimated from train+cal
    ...
    for cw in [None, "balanced"]:
        for C in settings.enet_c_values:
            for l1 in settings.enet_l1_values:
                ...
                # precompute calibrated probs on val
                # apply bucket calibrators
                p_cal=np.empty_like(raw_val, dtype=float)
                for i in range(raw_val.size):
                    b=bucket_name_from_ttc(ttc_val[i])
                    cal=calibs.get(b) or calibs.get("global")
                ...
                for alpha in settings.prior_alpha_values:
                    ...
                    for i in range(p_cal.size):
                        b=bucket_name_from_ttc(ttc_val[i])
                        prior=float(priors.get(b, priors.get("global", 0.5)))
    ...
    # choose best by Brier then AUC
    def key(c):
        return (c["brier"], -(c["auc"] if c["auc"] is not None else -1.0))
    best=min(candidates, key=key)

    # final fit: base on train+cal, calibrate on val (most recent) for drift control
    ...
    raw_val=pipe_final.decision_function(X_val)
    ttc_val=X_val[:, FEATURE_NAMES.index("ttc_frac")]
    ...
    bundle=ModelBundle(
        ...
        meta={
            ...
            "split_days": {"train": int(len(train_set)), "cal": int(len(cal_set)), "val": int(len(val_set))},
            "selected": {k: best[k] for k in ["cw","C","l1","alpha","brier","auc"]},
...
def run_training(settings: Settings, symbols: List[str]) -> Dict[str, object]:
    ...
    X,y1,y2,day_idx,err = build_training_dataset(
        client=client,
        symbols=symbols,
