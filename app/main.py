...
app = FastAPI(title="S&P 500 Prob Scanner", version="8.0.2")
...
def health() -> Dict[str, Any]:
    return {"ok": True, "service": "sp500-prob-scanner", "version": "8.0.2"}
...
def api_status(settings: Settings = Depends(get_settings)) -> Dict[str, Any]:
    ...
    snap["v8_tuning"] = {"calib_min_bucket_samples": settings.calib_min_bucket_samples, "enet_c_values": settings.enet_c_values, "enet_l1_values": settings.enet_l1_values, "prior_alpha_values": settings.prior_alpha_values}
    return snap
...
def _training_thread(settings: Settings) -> None:
    try:
        ...
        if not symbols:
            SCANNER.load_constituents(); symbols=[c.symbol for c in SCANNER.constituents]
        ...
        with STATE.lock:
            ...
            STATE.training.last_error = None
            pt1 = res.get("pt1", {}); pt2 = res.get("pt2", {})
            STATE.model.pt1.trained = True; STATE.model.pt1.path = os.path.join(settings.model_dir, "pt1"); STATE.model.pt1.auc_val = pt1.get("auc_val"); STATE.model.pt1.brier_val = pt1.get("brier_val"); STATE.model.pt1.calibrator = pt1.get("calibrator"); STATE.model.pt1.class_weight = pt1.get("class_weight"); STATE.model.pt1.alpha = pt1.get("alpha")
            STATE.model.pt2.trained = True; STATE.model.pt2.path = os.path.join(settings.model_dir, "pt2"); STATE.model.pt2.auc_val = pt2.get("auc_val"); STATE.model.pt2.brier_val = pt2.get("brier_val"); STATE.model.pt2.calibrator = pt2.get("calibrator"); STATE.model.pt2.class_weight = pt2.get("class_weight"); STATE.model.pt2.alpha = pt2.get("alpha")
        try:
