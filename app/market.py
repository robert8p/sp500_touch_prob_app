...
def get_market_times(now_utc: datetime, tz_name: str="America/New_York") -> Tuple[datetime, datetime, bool, int]:
    ...
    try:
        ...
        sched = cal.schedule(start_date=now_local.date() - timedelta(days=1), end_date=now_local.date() + timedelta(days=7))
        if len(sched) >= 1:
            for _, row in sched.iterrows():
                sess_open = row["market_open"].to_pydatetime().astimezone(tz)
                sess_close = row["market_close"].to_pydatetime().astimezone(tz)
                if now_local < sess_close:
                    open_local = sess_open
                    close_local = sess_close
            ...
            last_row = sched.iloc[-1]
            open_local = last_row["market_open"].to_pydatetime().astimezone(tz)
            close_local = last_row["market_close"].to_pydatetime().astimezone(tz)
        ...
        if now_local.weekday() >= 5:
            ...
            open_local = next_local.replace(hour=9, minute=30, second=0, microsecond=0)
            close_local = next_local.replace(hour=16, minute=0, second=0, microsecond=0)
            return open_local.astimezone(ZoneInfo("UTC")), close_local.astimezone(ZoneInfo("UTC")), False, 0
    is_open = (now_local >= open_local) and (now_local < close_local)
    ttc = int(max(0, (close_local - now_local).total_seconds())) if is_open else 0
    return open_local.astimezone(ZoneInfo("UTC")), close_local.astimezone(ZoneInfo("UTC")), is_open, ttc
