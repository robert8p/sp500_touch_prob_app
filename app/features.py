...
# ── v10: 4 new features appended ──
FEATURE_NAMES = [
    'ret_5m', 'ret_30m', 'rel_strength_spy_5m', 'rel_strength_spy_30m', 'rel_strength_sector_30m',
    ...
    'ema_diff_pct', 'adx', 'atr_pct', 'rv_1h', 'rvol_tod', 'obv_slope_norm',
    'vwap_loc', 'dist_pct_to_vwap', 'closes_above_vwap_frac_30m', 'vwap_slope_30m', 'donch_dist',
    ...
    'ttc_frac', 'log_mins_to_close', 'tod_frac', 'tod_sin', 'tod_cos',
    'dvol_usd_med_log', 'range_pct_med', 'wick_atr_med',
    'int_mom_rvol', 'int_mom_vwap', 'int_trend_adx', 'int_mom_ttc',
    # ── NEW in v10 ──
    'consecutive_higher_highs_5m',
    'drawdown_recovery_30m',
    'path_smoothness_15m',
    'rel_strength_spy_since_open',
    # ── NEW in v10 patch ──
    'directional_volume_ratio',
    'rvol_directional',
    # ── Blindspot features v10 ──
    'spy_ret_5m_raw',
    'spy_consecutive_down',
    'spy_rv_1h',
    'sector_ret_since_open',
    'extension_ratio',
    'intraday_rejection_count',
    'volume_trend_5bar',
    'rw_implied_touch_prob',
]

DIRECTIONAL_REASON_CAP = 14
...
def _directional_reasons(ret5, ret30, rel_spy5, rel_spy, rel_sector, ema_pct, adx_v, rvol_tod, vwap_loc, dist_pct_to_vwap, vwap_slope, donch, gap_prev_close, daily_ctx, ret_since_open, smoothness, reversal_count, or_state):
    codes = []
    if ret30 > 0.006:
        codes.append('MOM30+')
    if ret5 > 0.0025:
    ...
    if ema_pct < 0 and adx_v > 18:
        codes.append('TREND-')
    if vwap_loc > 0.3 or dist_pct_to_vwap > 0.002:
        codes.append('>VWAP')
    if vwap_loc < -0.3 or dist_pct_to_vwap < -0.002:
        codes.append('<VWAP')
    if vwap_slope > 0.001:
        codes.append('VWAP_UP')
    if rvol_tod > 1.4:
        codes.append('RVOL')
    if donch < 0.6:
...
def compute_features_from_5m(
    ...
    spy_ret_since_open: float = 0.0,     # NEW in v10
    spy_ret_5m_raw: float = 0.0,        # SPY absolute 5m return
    spy_consecutive_down: int = 0,       # SPY consecutive negative 5m bars
    spy_rv_1h: float = 0.0,             # SPY realized vol last hour
    sector_ret_since_open: float = 0.0,  # Sector ETF absolute since-open return
) -> Optional[FeatureRow]:
    ...
    rel_spy5 = float(ret5 - float(spy_ret_5m))
    ...
    dist_pct_to_vwap = float(price / vwap - 1.0) if vwap > 0 else 0.0
    look = min(20, h.size)
    ...
    recent_n = min(7, cur_c.size)
    recent_c = cur_c[-recent_n:]
    recent_vw = cur_vw[-recent_n:]
    closes_above_vwap_frac_30m = float(np.mean(recent_c >= recent_vw)) if recent_c.size else 0.0
    if cur_vw.size >= 7 and cur_vw[-7] > 0:
        vwap_slope_30m = float(cur_vw[-1] / cur_vw[-7] - 1.0)
    elif cur_vw.size >= 2 and cur_vw[0] > 0:
        vwap_slope_30m = float(cur_vw[-1] / cur_vw[0] - 1.0)
    else:
        vwap_slope_30m = 0.0
    ...
    tod_angle = 2.0 * math.pi * tod_frac
    tod_sin = float(math.sin(tod_angle))
    tod_cos = float(math.cos(tod_angle))
    ...
    dead_cat = (dist_pct_to_vwap < -0.003) and (ret_since_open < 0.0) and (rel_spy < -0.0025) and (damage_from_high_atr >= 1.8)
    broken_or = (or_breakout_state < -0.5) and open_weak and no_reclaim
    blocked_hits = int(lt_down) + int(open_weak) + int(no_reclaim) + int(damage) + int(broken_or) + int(dead_cat)
    if lt_down:
    ...
    int_mom_vwap = float(ret30 * max(dist_pct_to_vwap, vwap_loc))
    int_trend_adx = float(ema_diff_pct * adx_v)
    int_mom_ttc = float(ret30 * ttc_frac)

    # ── NEW FEATURES v10 ──

    # 1. Consecutive higher highs in 5m bars (look back up to 12 bars)
    chh = 0.0
    for k in range(2, min(13, cur_h.size + 1)):
        if cur_h[-k + 1] > cur_h[-k]:
            chh += 1.0
        else:
            break
    consecutive_higher_highs_5m = chh

    # 2. Drawdown recovery ratio over 30m: how much of the intraday dip has been recovered
    if cur_l.size >= 4:
        session_low = float(np.min(cur_l))
        dip = session_high - session_low
        recovery = price - session_low
        drawdown_recovery_30m = float(recovery / dip) if dip > 1e-6 else 1.0
    else:
        drawdown_recovery_30m = 1.0

    # 3. Path smoothness over 15m window (3 bars)
    if cur_c.size >= 4:
        ret_seq_15 = np.diff(cur_c[-4:]) / np.maximum(cur_c[-4:-1], 1e-9)
        abs_sum_15 = float(np.sum(np.abs(ret_seq_15)))
        ret_15m = float(price / cur_c[-4] - 1.0) if cur_c[-4] > 0 else 0.0
        path_smoothness_15m = float(ret_15m / (abs_sum_15 + 1e-9)) if abs_sum_15 > 0 else 0.0
    else:
        path_smoothness_15m = 0.0

    # 4. Relative strength vs SPY since open
    rel_strength_spy_since_open = float(ret_since_open - float(spy_ret_since_open))

    # 5. Directional volume ratio: up-bar volume / down-bar volume over last 30min
    recent_bars = min(6, cur_c.size)
    if recent_bars >= 2:
        rc = cur_c[-recent_bars:]
        rv = cur_v[-recent_bars:]
        up_vol = float(np.sum(rv[1:][rc[1:] > rc[:-1]])) if np.any(rc[1:] > rc[:-1]) else 0.0
        dn_vol = float(np.sum(rv[1:][rc[1:] < rc[:-1]])) if np.any(rc[1:] < rc[:-1]) else 0.0
        directional_volume_ratio = float(np.clip(up_vol / max(dn_vol, 1.0), 0.0, 5.0)) if (up_vol + dn_vol) > 0 else 1.0
    else:
        directional_volume_ratio = 1.0

    # 6. RVOL directional: positive when elevated volume accompanies upward move, negative on sell-off
    rvol_directional = float(np.clip(rvol_tod * (1.0 if ret30 >= 0 else -1.0), -5.0, 5.0))

    # ── BLINDSPOT FEATURES v10 ──

    # 7. Extension ratio: how far the stock has moved vs what's statistically normal
    bars_since_open = max(1.0, float(cur_c.size))
    expected_range = atr_pct * math.sqrt(bars_since_open) if atr_pct > 0 else 0.01
    extension_ratio = float(np.clip(abs(ret_since_open) / max(expected_range, 1e-6), 0.0, 5.0))

    # 8. Intraday rejection count: failed breakout attempts at session high
    intraday_rejection_count = 0.0
    if cur_h.size >= 3:
        session_hi = float(np.max(cur_h))
        threshold = session_hi * 0.999  # within 0.1%
        for k in range(cur_h.size):
            if cur_h[k] >= threshold and cur_c[k] < threshold:
                intraday_rejection_count += 1.0
    intraday_rejection_count = min(intraday_rejection_count, 8.0)

    # 9. Volume trend over last 5 bars: declining volume on rising price = exhaustion
    if cur_v.size >= 5:
        vol_recent = cur_v[-5:]
        vol_x = np.arange(5, dtype=float)
        vol_x = vol_x - vol_x.mean()
        vol_denom = float((vol_x * vol_x).sum())
        vol_slope = float((vol_x * (vol_recent - vol_recent.mean())).sum() / vol_denom) if vol_denom > 0 else 0.0
        med_vol = float(np.median(vol_recent)) if vol_recent.size else 1.0
        volume_trend_5bar = float(np.clip(vol_slope / max(med_vol, 1.0), -3.0, 3.0))
    else:
        volume_trend_5bar = 0.0

    # 10. Random walk implied touch probability (barrier option approximation)
    #     P(touch +1%) = exp(-2 * ln(1.01) * ln(1.01) / (2 * sigma^2 * T))
    #     Simplified: P ≈ 2 * N(-d) where d = target / (sigma * sqrt(T))
    remaining_bars = max(1.0, mins_to_close / 5.0)
    sigma_per_bar = rv_1h / max(1.0, math.sqrt(12.0)) if rv_1h > 0 else 0.001
    sigma_total = sigma_per_bar * math.sqrt(remaining_bars)
    if sigma_total > 1e-6:
        d = 0.01 / sigma_total  # +1% target in return space
        # Normal CDF approximation for barrier touch: P ≈ 2 * Phi(-d)
        # Using logistic approximation of Phi: Phi(x) ≈ 1/(1+exp(-1.7*x))
        phi_neg_d = 1.0 / (1.0 + math.exp(1.7 * d))
        rw_implied_touch_prob = float(np.clip(2.0 * phi_neg_d, 0.0, 1.0))
    else:
        rw_implied_touch_prob = 0.0

    feats = np.array([
        ret5, ret30, rel_spy5, rel_spy, rel_sector,
        gap_prev_close_pct, dist_prev_high_atr, dist_prev_low_atr, prev_day_range_pct,
        ret_5d, ret_20d, ret_60d, dist_20dma_pct, dist_50dma_pct, dist_200dma_pct, drawdown_20d_pct, drawdown_60d_pct,
        ema_diff_pct, adx_v, atr_pct, rv_1h, rvol_tod, obv_slope_norm,
        vwap_loc, dist_pct_to_vwap, closes_above_vwap_frac_30m, vwap_slope_30m, donch,
        ...
        ttc_frac, log_mins, tod_frac, tod_sin, tod_cos,
        dvol_usd_med_log, range_pct_med, wick_atr_med,
        int_mom_rvol, int_mom_vwap, int_trend_adx, int_mom_ttc,
        # ── NEW ──
        consecutive_higher_highs_5m,
        ...
        directional_volume_ratio,
        rvol_directional,
        float(spy_ret_5m_raw),
        float(spy_consecutive_down),
        float(spy_rv_1h),
        float(sector_ret_since_open),
        extension_ratio,
        intraday_rejection_count,
        volume_trend_5bar,
        rw_implied_touch_prob,
    ], dtype=float)
    reasons = _directional_reasons(ret5, ret30, rel_spy5, rel_spy, rel_sector, ema_diff_pct, adx_v, rvol_tod, vwap_loc, dist_pct_to_vwap, vwap_slope_30m, donch, gap_prev_close_pct, daily_ctx, ret_since_open, path_smoothness, reversal_count, or_breakout_state)
