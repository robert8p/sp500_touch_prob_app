...
FEATURE_NAMES = [
    "ret_5m","ret_30m","rel_strength_30m","ema_diff_pct","adx","atr_pct","rv_1h","rvol_tod",
    "obv_slope_norm","vwap_loc","donch_dist","ttc_frac","log_mins_to_close","tod_frac",
    "dvol_usd_med_log","range_pct_med","wick_atr_med","int_mom_rvol","int_mom_vwap","int_trend_adx","int_mom_ttc",
]
...
def _risk_and_micro(o, h, l, c, v, atr_arr, rolling_bars, dvol_min_usd, range_pct_max, wick_atr_max):
    ...
    dvol_series=c_w*np.maximum(v_w,0.0); dvol_med=float(np.median(dvol_series)) if dvol_series.size else 0.0
    rng_series=np.where(c_w>0, (h_w-l_w)/c_w, 0.0); rng_med=float(np.median(rng_series)) if rng_series.size else 0.0; rng_cur=float((h[-1]-l[-1])/c[-1]) if c[-1] else 0.0
    upper=h_w-np.maximum(o_w,c_w); lower=np.minimum(o_w,c_w)-l_w; wick=np.maximum(upper,lower); wick_norm=np.where(atr_w>0, wick/atr_w, 0.0); wick_med=float(np.median(wick_norm)) if wick_norm.size else 0.0
    atr_cur=float(atr_arr[-1]) if atr_arr.size else 0.0; upper_c=float(h[-1]-max(o[-1],c[-1])); lower_c=float(min(o[-1],c[-1])-l[-1]); wick_cur=max(upper_c,lower_c); wick_cur_n=float(wick_cur/atr_cur) if atr_cur>0 else 0.0
...
def _directional_reasons(ret30: float, rel: float, ema_pct: float, adx_v: float, rvol_tod: float, vwap_loc: float, donch: float) -> str:
    codes=[]
...
def compute_features_from_5m(bars_5m, spy_ret_30m, mins_to_close, tod_baseline_vol_median, rolling_rvol_window, risk_params):
    ...
    ema_fast=ema(c, span=6); ema_slow=ema(c, span=18)
    ema_diff_pct=float((ema_fast[-1]-ema_slow[-1])/price)
    adx_v=float(adx(h,l,c,period=14)[-1])
    atr_arr=atr(h,l,c,period=14); atr_v=float(atr_arr[-1])
    atr_pct=float(atr_v/price) if atr_v>0 else 0.0
    logrets=np.diff(np.log(np.maximum(c,1e-9))); seg=logrets[-12:] if logrets.size>=12 else logrets
    rv_1h=float(np.std(seg)*math.sqrt(max(1.0,float(seg.size)))) if seg.size>2 else 0.0
    current_vol=float(v[-1]); prev_v=v[:-1]; med_prev=float(np.median(prev_v[-rolling_rvol_window:])) if prev_v.size>0 else 0.0
    ...
    if tod_baseline_vol_median is not None and tod_baseline_vol_median>0:
        rvol_tod=float(current_vol/tod_baseline_vol_median); used_profile=True
    else:
        rvol_tod=float(fallback_rvol)
    obv_series=obv(c,v); obv_sl=slope_last(obv_series, window=12); obv_slope_norm=float(obv_sl/max(1.0, med_prev))
    vwap_loc=float((price-vwap)/atr_v) if atr_v>0 else 0.0
    look=min(20, h.size); hi=float(np.max(h[-look:])) if look>0 else float(h[-1]); donch=float((hi-price)/atr_v) if atr_v>0 else 0.0
    ttc_frac=float(np.clip(mins_to_close/390.0, 0.0, 1.0)); log_mins=float(math.log1p(max(0.0, mins_to_close))); tod_frac=float(np.clip((390.0-mins_to_close)/390.0, 0.0, 1.0))
    ...
    dvol_usd_med_log=float(math.log1p(max(0.0, dvol_med))); range_pct_med=float(max(0.0, rng_med)); wick_atr_med=float(max(0.0, wick_med))
    int_mom_rvol=float(ret30*(rvol_tod-1.0)); int_mom_vwap=float(ret30*vwap_loc); int_trend_adx=float(ema_diff_pct*adx_v); int_mom_ttc=float(ret30*ttc_frac)
    feats=np.array([ret5, ret30, rel_strength, ema_diff_pct, adx_v, atr_pct, rv_1h, rvol_tod, obv_slope_norm, vwap_loc, donch, ttc_frac, log_mins, tod_frac, dvol_usd_med_log, range_pct_med, wick_atr_med, int_mom_rvol, int_mom_vwap, int_trend_adx, int_mom_ttc], dtype=float)
    reasons=_directional_reasons(ret30, rel_strength, ema_diff_pct, adx_v, rvol_tod, vwap_loc, donch)
    risk_reasons=" ".join(risk_codes)
