...
def _risk_flag(
    ...
    # Use rolling medians (exclude current bar for stability)
    n = c.size
    if n < 2:
        return "OK", []
    ...
    # current bar wickiness (normalized by current ATR)
...
def _reason_codes(ret30: float, rel: float, ema_pct: float, adx_v: float, rvol_tod: float, vwap_loc: float, donch: float, risk_codes: List[str]) -> str:
    ...
    # append risk codes (ensure they appear)
...
def compute_features_from_5m(
    ...
    # Fallback rolling median excludes current bar to avoid current-bar leakage
    prev = v[:-1]
    med_prev = float(np.median(prev[-rolling_rvol_window:])) if prev.size > 0 else 0.0
