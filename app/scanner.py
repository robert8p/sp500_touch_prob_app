...
class Scanner:
    ...
    def load_constituents(self) -> None:
        ...
        if refreshed is not None:
            data=refreshed; source="wikipedia"
        elif err:
    ...
    def _demo_scores(self) -> List[ScoreRow]:
        ...
        for i,(sym,sec) in enumerate(demo):
            price=100+i*20; vwap=price-0.4; p2=min(0.95, 0.55+0.08*i); p1=min(0.98, p2+0.15)
    ...
    def scan_once(self, open_utc: datetime, close_utc: datetime, now_utc: datetime) -> None:
        ...
        if self.settings.demo_mode:
            with self.state.lock:
                self.state.alpaca.ok=True; self.state.alpaca.message="DEMO_MODE"; self.state.alpaca.feed="sip"; self.state.alpaca.last_request_utc=run_utc; self.state.alpaca.last_bar_timestamp=run_utc; self.state.alpaca.rate_limit_warn=None
            rows=self._demo_scores(); cov.symbols_scored_count=len(rows); cov.top_skip_reasons={}
        ...
        if client is None:
            ...
            with self.state.lock:
                self.state.alpaca.ok=False; self.state.alpaca.message="Missing keys"; self.state.alpaca.feed=self.settings.normalized_feed()
        ...
        with self.state.lock:
            ...
            if err:
                self.state.alpaca.ok=False; self.state.alpaca.message=err
            else:
                self.state.alpaca.ok=True; self.state.alpaca.message="OK"
        if err:
        ...
        if spy_bars and len(spy_bars) >= 7:
            c=float(spy_bars[-1].get("c") or 0.0); c0=float(spy_bars[-7].get("c") or c); spy_ret_30m=(c/c0-1.0) if c0 else 0.0
        avail,_=self.vol_profiles.availability_counts(); cov.profile_symbols_available=avail; cov.profile_symbols_missing=max(0, cov.universe_count-avail)
        if avail == 0:
        ...
        for sym in universe_symbols:
            ...
            if not bars:
                skip_counts["no_bars"] += 1; skipped.append(SkippedSymbol(symbol=sym, reason="no_bars", last_bar_timestamp=None)); continue
            if len(bars) < self.settings.min_bars_5m:
                skip_counts["insufficient_bars"] += 1; skipped.append(SkippedSymbol(symbol=sym, reason="insufficient_bars", last_bar_timestamp=bars[-1].get("t"))); continue
            sufficient_count += 1
            last_ts=bars[-1].get("t"); slot_idx=None
            if last_ts:
                try:
            ...
            try:
                fr=compute_features_from_5m(bars_5m=bars, spy_ret_30m=spy_ret_30m, mins_to_close=mins_to_close, tod_baseline_vol_median=baseline, rolling_rvol_window=20, risk_params=risk_params)
            except Exception:
                fr=None
            if fr is None:
                skip_counts["missing_price_or_vwap"] += 1; skipped.append(SkippedSymbol(symbol=sym, reason="missing_price_or_vwap", last_bar_timestamp=last_ts)); continue
        ...
        if not feats:
            ...
            self.state.set_scores([], run_utc); self.state.set_coverage(cov, skipped); return
        X=np.vstack(feats)
        p1, src1, status1 = predict_probs(self.settings.model_dir, X, 1)
        p2, src2, status2 = predict_probs(self.settings.model_dir, X, 2)
        if (status1=="incompatible") or (status2=="incompatible"):
        ...
        with self.state.lock:
            self.state.model.pt1.trained=(src1=="trained"); self.state.model.pt1.path=os.path.join(self.settings.model_dir, "pt1")
            self.state.model.pt2.trained=(src2=="trained"); self.state.model.pt2.path=os.path.join(self.settings.model_dir, "pt2")
        rows=[]
        for i,(sym,sector,price,vwap,risk,risk_reasons,reasons) in enumerate(meta):
            rows.append(ScoreRow(sym, sector, float(price), float(vwap), float(p1[i]), float(p2[i]), risk, risk_reasons, reasons))
        rows.sort(key=lambda r: r.prob_2, reverse=True)
        cov.symbols_scored_count=len(rows); cov.top_skip_reasons={k:v for k,v in skip_counts.items() if v>0}
        self.state.set_scores(rows, run_utc); self.state.set_coverage(cov, skipped)
        try:
    ...
    def _loop(self) -> None:
        try:
            ...
            with self.state.lock:
                self.state.constituents.source="fallback"; self.state.constituents.warning=f"failed to load constituents: {e}"
