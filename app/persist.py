...
def ai_strategy_state_path(model_dir: str) -> str:
    return os.path.join(model_dir, 'ai_strategy_last.json')


def save_ai_strategy_last(model_dir: str, payload: Dict[str, Any]) -> None:
    os.makedirs(model_dir, exist_ok=True)
    payload = dict(payload)
    payload.setdefault('saved_at_utc', _utc_now())
    with open(ai_strategy_state_path(model_dir), 'w', encoding='utf-8') as f:
        json.dump(payload, f)


def load_ai_strategy_last(model_dir: str) -> Optional[Dict[str, Any]]:
    p = ai_strategy_state_path(model_dir)
    if not os.path.exists(p):
        return None
    try:
        with open(p, 'r', encoding='utf-8') as f:
